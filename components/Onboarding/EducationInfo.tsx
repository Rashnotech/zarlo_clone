...
export default function EducationInfo({
  ...
  const initialSpecialities =
    beauticianProfile.otherSpecialties.length > 0
  ...
  const initialDocs = beauticianProfile.boardCertificates.map((item) => {
    return {
      title: item,
      size: 0,
      url: item,
  ...
  const {
    ...
    defaultValues: {
      medicalSchool:
        beauticianProfile.medicalSchool || savedDBData.medicalSchool,
      graduationYear:
        beauticianProfile.graduationYear || savedDBData.graduationYear,
      primarySpecialization:
        beauticianProfile.primarySpecialization ||
      ...
      <form className=" py-4 px-4  mx-auto " onSubmit={handleSubmit(onSubmit)}>
        <div className="grid gap-4 grid-cols-2">
          ...
          <MultipleFileUpload
            ...
            files={docs as any}
            setFiles={setDocs}
            endpoint="beauticianProfessionDocs"
          />
        </div>
