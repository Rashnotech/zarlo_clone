...
import { completeProfile, updateBeauticianProfile } from "@/actions/onboarding";
...
export default function AdditionalInfo({
  ...
  const initialDocs = beauticianProfile.additionalDocs.map((item) => {
    return {
      title: item,
      size: 0,
      url: item,
  ...
  const {
    ...
    defaultValues: {
      educationHistory:
        beauticianProfile.educationHistory || savedDBData.educationHistory,
      research: beauticianProfile.research || savedDBData.research,
      accomplishments:
        beauticianProfile.accomplishments || savedDBData.accomplishments,
  ...
  async function onSubmit(data: additionalFormProps) {
    ...
    try {
      if (isOnboarding) {
        const res = await completeProfile(beauticianProfile.id, data);
    ...
    <div className="w-full">
      ...
      <form className=" py-4 px-4  mx-auto " onSubmit={handleSubmit(onSubmit)}>
        <div className="grid gap-4 grid-cols-2">
          ...
          />
          <MultipleFileUpload
            label="Any Additional Documents (ID, CV, Certifications, etc.) Upload"
