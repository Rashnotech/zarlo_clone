...
import { BeauticianStatus } from "@prisma/client";
...
export default function ApproveBtn({
  ...
  async function updateStatus() {
    ...
    // console.log(data);
    try {
      const res = await updateBeauticianProfile(profileId, data);
      if (res?.status === 201) {
        toast.success("Beautician Status Changed Successfully");
        setLoading(false);
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Approve the Beautician</DialogTitle>
          <DialogDescription>
            <div className="py-4">
              <ShadSelectInput
                label="Status"
