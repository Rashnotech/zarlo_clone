...
import { getBeauticianAnalytics } from "@/actions/stats";
...
export default async function BeauticianDashboard({
  ...
  const beauticianId = user?.id ?? "";
  const beauticianProfile = (await getBeauticianProfileById(beauticianId))
    ?.data;
  const analytics = (await getBeauticianAnalytics()) || [];
  const appointments =
    (await getBeauticianAppointments(beauticianId)).data || [];
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Button asChild>
                <Link href="/dashboard/beautician/appointments">View All</Link>
              </Button>
            </div>
          </CardHeader>
                  ...
                  <div key={client.email} className="flex items-center gap-4">
                    ...
                    <div className="ml-auto font-medium flex space-x-2 items-center">
                      <Button size={"sm"} asChild variant={"outline"}>
                        <Link
                          href={`/dashboard/beautician/clients/view/${client.clientId}`}
                        >
                          View
                        </Link>
                      </Button>
