  ...
  try {
    const services = [
      ...
      },
      {
        ...
        slug: "in-person-beautician-visit",
        imageUrl:
...
export interface ServiceWithBeauticianProfileCount {
  id: string;
  title: string;
  slug: string;
  imageUrl: string;
  _count: {
    beauticianProfiles: number;
  };
}
...
export async function updateBeauticianProfileWithService(
  id: string | undefined,
  data: any
) {
  if (id) {
    try {
      ...
      revalidatePath("/dashboard/beautician/settings");
      return {
        data: updatedProfile,
        status: 201,
        error: null,
