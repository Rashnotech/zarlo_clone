...
export async function createBeauticianProfile(formData: any) {
  const {
    dob,
    firstName,
    gender,
  ...
  try {
    const newProfile = await prismaClient.beauticianProfile.create({
      data: {
        dob,
        firstName,
        gender,
...
export async function updateBeauticianProfile(id: string | undefined, data: any) {
  if (id) {
    try {
      const updatedProfile = await prismaClient.beauticianProfile.update({
        where: {
          id,
        },
        data,
...
export async function getApplicationByTrack(trackingNumber: string) {
  if (trackingNumber) {
    try {
      const existingProfile = await prismaClient.beauticianProfile.findUnique({
        where: {
          trackingNumber,
        },
      });
...
export async function completeProfile(id: string | undefined, data: any) {
  ...
  if (id) {
    try {
      ...
      const previewText = "Welcome to ZARLO ";
      const message =
        "Thank you for joining the ZARLO app, we are so grateful that we have onboard ";
      const sendMail = await resend.emails.send({
        from: "Zarlo App <info@zarloapp.com>",
        to: email,
        subject: "Welcome to ZARLO",
...
export async function getBeauticianProfileById(userId: string | undefined) {
  if (userId) {
    try {
